...
class FunctionCosinusIntegrationTest {
    private val sin = SinusImpl()
    private val ln = NaturalLogarithmTableImpl()
    private val cos = CosinusImpl(sin)
    private val sec = SecantTableImpl()
    private val tan = TangentTableImpl()
    private val cot = CotangentTableImpl()
    ...
    fun `Given only sinus module When computing function value at F(x) = 0 Then F(x) is correct`() {
        assertEquals(-2.255, f(2 * (Math.PI * 0.0 - 1.31766), epsilon), diff)
        assertEquals(-2.255, f(2 * (Math.PI * -1.0 - 1.31766), epsilon), diff)
        assertEquals(0.006, f(2 * (Math.PI * 0.0 - 0.1999472), epsilon), diff)
        assertEquals(0.006, f(2 * (Math.PI * -1.0 - 0.1999472), epsilon), diff)
        assertEquals(-2.255, f(2 * (Math.PI * -1.0 + 1.31522), epsilon), diff)
        assertEquals(-2.255, f(2 * (Math.PI * -2.0 + 1.31522), epsilon), diff)
    }
